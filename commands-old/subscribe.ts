...
Command.code = async (client, message: Message) => {
  ...
  if(!message.member.permissionsIn(message.channel.id).has('MANAGE_WEBHOOKS') && !message.member.permissions.has('MANAGE_GUILD') && message.member.id!=="190916650143318016")
  ...
  if(!message.guild.me.permissionsIn(message.channel.id).has('MANAGE_WEBHOOKS'))
  ...
  await (client.channels.cache.get('692495015322189934') as NewsChannel).addFollower(message.channel.id, `${message.author.tag} used the subscribe command`).catch(e => {message.channel.send({content: '<:no:424361302069346304> '+e.toString(), allowedMentions: {parse: []}}); error = true})
